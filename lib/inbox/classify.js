...
// Our own system reports; everything else from our team still gets read,
// because forwarded leads (Fwd: inquiry, Drift chat contacts) come from us.
const INTERNAL_REPORT = /daily report|digest|deal hub|weekly insights|test\b/i;

export function preFilter(fromEmail = "", subject = "") {
  const f = fromEmail.trim().toLowerCase();
  if (INTERNAL.test(f)) return INTERNAL_REPORT.test(subject || "") ? { kind: "internal", source: "email" } : { source: "email", internal: true };
...
const KINDS = ["offer", "seller_lead", "buyer_question", "data_room", "existing_deal", "marketplace_lead", "vendor", "spam", "internal", "other"];

// routes: [{ slug, title, dl_number }]
export async function classifyEmail(email, routes, { isMarketplace = false, isInternal = false, ddItems = [] } = {}) {
...
- internal: our own team talking to each other (no outside inquiry)
- other: anything else
${isMarketplace ? "This came from a marketplace lead service: use marketplace_lead unless it is clearly something else.\n" : ""}${isInternal ? "This was sent by someone on our own team. If it forwards or passes along an outside buyer's or seller's inquiry (e.g. \"Fwd:\", chat/Drift contacts, a lead), classify it as that inquiry. If it's just internal team chatter, use kind \"internal\".\n" : ""}
