...
export default function DealHub() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Load Q&A — questions with at least one answer for this deal
        loadQA(dealId);
  ...
  const loadQA = async (dealId) => {
    try {
      // Fetch answered questions for this deal
      const answers = await api(
        "deal_answers?deal_id=eq." + dealId +
        "&is_deleted=eq.false&select=id,answered_by_name,answered_by_role,answer,answered_at,question_id" +
      ...
      const qMap = {};
      (questions || []).forEach(q => { qMap[q.id] = { ...q, answers: [] }; });
      (answers || []).forEach(a => { if (qMap[a.question_id]) qMap[a.question_id].answers.push(a); });

      const items = Object.values(qMap).filter(q => q.answers.length > 0);
      setQaItems(items);

      // Open first 3 by default
      const defaultOpen = {};
      items.slice(0, 3).forEach(q => { defaultOpen[q.id] = true; });
      setOpenQa(defaultOpen);
    } catch (e) {
      console.error("Q&A load failed:", e);
    }
  };
  ...
  const toggleQa = (id) => setOpenQa(prev => ({ ...prev, [id]: !prev[id] }));

  const ROLE_LABEL = { advisor: "Advisor", seller: "Seller", ai: "AI" };
  const ROLE_COLOR = {
    advisor: { bg: "#dbeafe", color: "#1e40af" },
    seller:  { bg: "#dcfce7", color: "#166534" },
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: 40, maxWidth: 420, width: "100%", margin: "0 24px", boxShadow: "0 8px 40px rgba(0,0,0,0.3)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div>
            ...
            <input value={gateInput.name} onChange={e => setGateInput({ ...gateInput, name: e.target.value })} placeholder="John Smith" style={{ width: "100%", marginTop: 4, padding: "10px 12px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 14, outline: "none", boxSizing: "border-box" }} />
          </div>
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: 0.5 }}>Email Address *</label>
            <input value={gateInput.email} onChange={e => setGateInput({ ...gateInput, email: e.target.value })} onKeyDown={e => e.key === "Enter" && submitGate()} placeholder="john@example.com" type="email" style={{ width: "100%", marginTop: 4, padding: "10px 12px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 14, outline: "none", boxSizing: "border-box" }} />
          </div>
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: 0.5 }}>Phone Number</label>
            <input value={gateInput.phone} onChange={e => setGateInput({ ...gateInput, phone: e.target.value })} onKeyDown={e => e.key === "Enter" && submitGate()} placeholder="555-123-4567" type="tel" style={{ width: "100%", marginTop: 4, padding: "10px 12px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 14, outline: "none", boxSizing: "border-box" }} />
          </div>
          {gateError && <p style={{ color: "#ef4444", fontSize: 13, margin: 0 }}>{gateError}</p>}
          <button onClick={submitGate} disabled={gateSubmitting || !gateInput.email.trim()} style={{ marginTop: 4, padding: "12px", background: gateSubmitting || !gateInput.email.trim() ? "#94a3b8" : "#1e40af", color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 15, cursor: gateSubmitting || !gateInput.email.trim() ? "not-allowed" : "pointer" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "32px 24px" }}>
        ...
        {/* Documents */}
        <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 8px rgba(0,0,0,0.06)", overflow: "hidden", marginBottom: 20 }}>
        ...
        {qaItems.length > 0 && (
          <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 8px rgba(0,0,0,0.06)", overflow: "hidden", marginBottom: 20 }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #e2e8f0" }}>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: "#1e293b", margin: 0 }}>Buyer Q&A</h2>
              <p style={{ fontSize: 13, color: "#64748b", margin: "4px 0 0" }}>
                {qaItems.length} question{qaItems.length !== 1 ? "s" : ""} answered · visible to all buyers in this deal
              </p>
            </div>
            <div>
              {qaItems.map(function(q, qi) {
                const isOpen = openQa[q.id];
                return (
                  <div key={q.id} style={{ borderBottom: qi < qaItems.length - 1 ? "1px solid #f1f5f9" : "none" }}>
                    {/* Question row */}
                    <div
                      onClick={() => toggleQa(q.id)}
                      style={{ padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", cursor: "pointer", gap: 16 }}
                      onMouseOver={function(e) { e.currentTarget.style.background = "#f8fafc"; }}
                      onMouseOut={function(e) { e.currentTarget.style.background = "transparent"; }}
                    >
                      <div style={{ fontWeight: 600, color: "#1e293b", fontSize: 14, lineHeight: 1.4, flex: 1 }}>{q.question}</div>
                      <div style={{ color: "#94a3b8", fontSize: 18, flexShrink: 0, lineHeight: 1, transition: "transform 0.2s", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)", display: "inline-block" }}>›</div>
                    </div>
                    ...
                    {isOpen && (
                      <div style={{ padding: "0 24px 16px" }}>
                        {q.answers.map(function(a) {
                          const rc = ROLE_COLOR[a.answered_by_role] || ROLE_COLOR.advisor;
                          return (
                            <div key={a.id} style={{ marginBottom: 10, padding: "12px 14px", background: "#f8fafc", borderLeft: "3px solid #e2e8f0", borderRadius: "0 8px 8px 0" }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                                <span style={{ background: rc.bg, color: rc.color, fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 3, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                                  {ROLE_LABEL[a.answered_by_role] || "Team"}
                                </span>
                                <span style={{ fontSize: 12, color: "#64748b" }}>
                                  {a.answered_by_name} · {new Date(a.answered_at).toLocaleDateString()}
                                </span>
                              </div>
                              <p style={{ margin: 0, fontSize: 14, color: "#334155", lineHeight: 1.6 }}>{a.answer}</p>
                            </div>
                          );
                        })}
                      </div>
            ...
            <div style={{ padding: "14px 24px", borderTop: "1px solid #f1f5f9", background: "#f8fafc" }}>
              <button onClick={function() { setChatOpen(true); }} style={{ background: "none", border: "none", color: "#3b82f6", fontSize: 13, fontWeight: 600, cursor: "pointer", padding: 0 }}>
                + Ask a question →
              </button>
            </div>
          </div>
