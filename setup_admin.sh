...
mkdir -p app/admin
mkdir -p app/api/admin/ingest
mkdir -p app/api/admin/ingest-faq
mkdir -p app/api/admin/token

###############################################
# ADMIN INGEST API
###############################################
cat > app/api/admin/ingest/route.js << 'ENDOFFILE'
...
ENDOFFILE
echo "✅ app/api/admin/ingest/route.js"

###############################################
# ADMIN FAQ INGEST API
###############################################
cat > app/api/admin/ingest-faq/route.js << 'ENDOFFILE'
import { createClient } from "@supabase/supabase-js";
const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL || "https://wgrmxhxozoyvcmvbfuxv.supabase.co", process.env.SUPABASE_SERVICE_ROLE_KEY);
...
ENDOFFILE
echo "✅ app/api/admin/ingest-faq/route.js"

###############################################
# ADMIN TOKEN API
###############################################
cat > app/api/admin/token/route.js << 'ENDOFFILE'
import { createClient } from "@supabase/supabase-js";
...
ENDOFFILE
echo "✅ app/api/admin/token/route.js"

###############################################
# ADMIN PAGE
###############################################
cat > app/admin/page.js << 'ENDOFPAGEFILE'
"use client";
...
ENDOFPAGEFILE
echo "✅ app/admin/page.js"

echo ""
echo "=========================================="
echo "ADMIN PANEL READY! Push and visit /admin"
echo "=========================================="
